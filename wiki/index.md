---
title: Home
description: Booru Abuse wiki
---

Booru Abuse _(abbreviation suggestions requested&mdash;my personal submission is 'BBUSE')_ is an NPM package intent on fleshing out what could be the REST API of each applied booru or imageboard.

> [!NOTE]
> This is currently my form of 'concept art.' I'm only writing down how I want things to work here, not necessarily how they _do_ work _now._
> 
> That means currently, anything and probably everything on this wiki is merely conceptualized. Things you find may be inaccurate or yet to be tangible. It will be announced when a release is available.

## Features

The main appetizer is that it nabs exclusive info from _every_ source possible and applicable, including the front-end pages themselves.

This is particularly useful for Rule34.xxx, as their APIs are confusing and difficult to use, even with proper knowledge on how to use the given results.

## Supported sites

- [Rule34](./Rule34) &mdash; If it exists, there is porn of it.

## Links

- [github.com/chipfucker/booru-abuse](<https://github.com/chipfucker/booru-abuse>)